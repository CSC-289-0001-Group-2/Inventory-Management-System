...
read_inventory :: proc(file_name: string) -> []InventoryItem {
    ...
    count := mem.copy(transmute(^i32)&count_bytes, count_bytes[:])
...
test_database :: proc(handle: os.Handle) {
    ...
    success := write_item(handle, new_item1)
    if success {
    ...
    success = write_item(handle, new_item3)
    if success {
...
test_write_and_read_item :: proc() {
    ...
    success, read_items := read_inventory_items(handle)
    items_success, items := read_inventory_items(handle)
    success, items := read_inventory_items(handle)
    ...
    assert(items.id == item.id, "Item ID mismatch")
    assert(items.name.count == item.name.count, "Name count mismatch")
...
main :: proc() {
    ...
    // Read the file
    // Declare and initialize the default allocator
    default_allocator := mem.default_allocator()
    ...
    // Open a file handle for testing purposes
    handle, err := os.open(file_name, os.O_RDWR | os.O_CREATE, 0666)
    if err != nil {
        fmt.println("Failed to open file:", err)
        return
    }
